...
module.exports = function(app) {
  ...
  rootRouter
    .get('/', function *(next) {
      const dist = [].concat(rootRouter.stack, sessionRouter.stack);
      const res = [];
      dist.forEach(router => {
        res.push(`${router.path}#[${router.methods.join('|')}]`);
      });
      const temp = _.sortBy(res, string => {
        return string.length;
      });
      const num = temp[temp.length - 1].length;
    ...
    .post('/wd/hub/extra', controllers.extra.extraFunc)
  ...
  sessionRouter
    ...
    .get('/element/:elementId/screenshot', controllers.element.takeElementScreenshot)
    ...
    // cookie
    .get('/cookie/:name', controllers.cookie.getNamedCookie)
    .get('/cookie', controllers.cookie.getAllCookies)
    .post('/cookie', controllers.cookie.addCookie)
    .del('/cookie/:name', controllers.cookie.deleteCookie)
    .del('/cookie', controllers.cookie.deleteAllCookies)
    // url
    ...
    .post('/keyboard', controllers.keyboard)
    .post('/mouse', controllers.mouse);
